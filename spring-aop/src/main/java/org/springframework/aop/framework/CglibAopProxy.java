...
class CglibAopProxy implements AopProxy, Serializable {
	...
	public Object getProxy(@Nullable ClassLoader classLoader) {
		...
		try {
			...
			if (ClassUtils.isCglibProxyClass(rootClass)) {//className contains $$
			...
			if (classLoader != null) {
				...
				if (classLoader instanceof SmartClassLoader &&
						((SmartClassLoader) classLoader).isClassReloadable(proxySuperClass)) {//reload --> useCache(false)
			...
			Callback[] callbacks = getCallbacks(rootClass);//MethodInterceptor
		...
		public Object intercept(Object proxy, Method method, Object[] args, MethodProxy methodProxy) throws Throwable {
			...
			try {
				if (this.advised.exposeProxy) {
					...
					oldProxy = AopContext.setCurrentProxy(proxy);//实现其实是ThreadLocal<T>
				...
				if (chain.isEmpty() && Modifier.isPublic(method.getModifiers())) {//没有chain拦截器，并且方法的签名符合要求
	...
	private static class ProxyCallbackFilter implements CallbackFilter {
		...
		public int accept(Method method) {
			if (AopUtils.isFinalizeMethod(method)) {//method是析构方法finalize
				logger.trace("Found finalize() method - using NO_OVERRIDE");
				return NO_OVERRIDE;
			}
			if (!this.advised.isOpaque() && method.getDeclaringClass().isInterface() &&
					method.getDeclaringClass().isAssignableFrom(Advised.class)) {//method是Advised声明的接口方法
				if (logger.isTraceEnabled()) {
			...
			List<?> chain = this.advised.getInterceptorsAndDynamicInterceptionAdvice(method, targetClass);//一系列的增强
