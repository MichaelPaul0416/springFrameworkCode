	...
	public int registerBeanDefinitions(Map<?, ?> map, @Nullable String prefix, String resourceDescription)
		...
		for (Object key : map.keySet()) {
			...
			if (keyString.startsWith(prefix)) {
				...
				String nameAndProperty = keyString.substring(prefix.length());//指定前缀，如果为null的话就是空字符串-->""
				// Find dot before property name, ignoring dots in property keys.
				int sepIdx = -1;
				int propKeyIdx = nameAndProperty.indexOf(PropertyAccessor.PROPERTY_KEY_PREFIX);//xxx[]-->中xxx代表beanName
				if (propKeyIdx != -1) {
	...
	protected void registerBeanDefinition(String beanName, Map<?, ?> map, String prefix, String resourceDescription)
		...
		for (Map.Entry<?, ?> entry : map.entrySet()) {
			...
			if (key.startsWith(prefix + SEPARATOR)) {
				String property = key.substring(prefix.length() + SEPARATOR.length());//前缀+.,其实就是beanName+.
